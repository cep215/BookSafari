...
;( function() {
    ...
    Tree.prototype = {
        ...
        positionNodes: function() {
            ...
            for ( i = 0, len = this.nodeDB.db.length; i < len; i++ ) {
                ...
                if (node.id !== 0 && !(node.parent().id === 0 && this.CONFIG.hideRootNode)) {
                    ...
                    if(node.connStyle.style.stroke === 'green' || node.connStyle.style.stroke === 'red')
            ...
            for ( i = 0, len = highlightedNodes.length; i < len; i++ ) {
                highlightNode = highlightedNodes[i];
                this.setConnectionToParent(highlightNode, hidePoint);
            }
        ...
        setConnectionToParent: function( treeNode, hidePoint ) {
            ...
            connLine.attr( treeNode.connStyle.style );
    ...
    TreeNode.prototype = {
        ...
        reset: function( nodeStructure, id, parentId, tree, stackParentId ) {
            ...
            this.onmouseout = nodeStructure.onmouseout;
            this.onmouseover = nodeStructure.onmouseover;
    ...
    TreeNode.prototype.createGeometry = function( tree ) {
        ...
        if ( this.onmouseover ) {
            node.setAttribute('onmouseover', this.onmouseover);
        }

        if ( this.onmouseout ) {
            node.setAttribute('onmouseout', this.onmouseout);
        }
